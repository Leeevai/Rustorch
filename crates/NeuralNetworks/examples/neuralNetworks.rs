...
mod integration_tests {
    ...
    #[test]
    fn test_xor_like_problem() -> Result<(), Box<dyn std::error::Error>> {
        // Create a network suitable for XOR-like problems
        let mut nn = nn![2, 4, 1; Tanh]?;
        nn.xavier_initialization()?;

        // Test inputs (XOR pattern)
        let inputs = vec![
        ...
        // Forward propagation for all inputs
        for (i, input) in inputs.iter().enumerate() {
            let output = nn.forward(input)?;
            println!("Input {}: [{:.0}, {:.0}] -> Output: {:.4}", 
                     i, input.get(0, 0)?, input.get(1, 0)?, output.get(0, 0)?);
        }
    ...
    #[test]
    fn test_multi_output_classification() -> Result<(), Box<dyn std::error::Error>> {
        // Create a network for multi-class classification
        let mut nn = nn![4, 8, 6, 3; ReLU]?;
        nn.he_initialization()?;

        // Test with random input
        let input = Matrix::from_vec(4, 1, vec![0.5, -0.3, 0.8, 0.1])?;
        let output = nn.forward(&input)?;

        assert_eq!(output.dimensions(), (3, 1));
        
        // Could apply softmax here for probabilities
        println!("Classification output: [{:.4}, {:.4}, {:.4}]", 
    ...
    #[test]
    fn test_deep_network() -> Result<(), Box<dyn std::error::Error>> {
        // Test a deeper network
        let mut nn = nn![5, 10, 8, 6, 4, 2; LeakyReLU::default()]?;
        nn.he_initialization()?;

        let input = Matrix::from_vec(5, 1, vec![1.0, -0.5, 0.3, 0.8, -0.2])?;
        let output = nn.forward(&input)?;

        assert_eq!(output.dimensions(), (2, 1));
        assert_eq!(nn.num_layers(), 5);
        assert!(nn.parameter_count() > 100); // Should have many parameters

        println!("Deep network output: [{:.4}, {:.4}]", 
